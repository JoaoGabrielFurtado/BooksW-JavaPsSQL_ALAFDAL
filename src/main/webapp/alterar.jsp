...
<%
    ...
    umLivro.setAnoEdicao(request.getParameter("anoedicao"));
    umLivro.setLocalizacao(request.getParameter("localizacao"));
    
    Codigo.Livro chaves = new Codigo.Livro();
    chaves.setTitulo(request.getParameter("titulo"));
    
    try {
        Codigo.ALDAL.update(umLivro, chaves);
        
        if (Codigo.Erro.getErro()) {
            temErro = true;
            mensagemH1 = "Ocorreu um erro ao atualizar:";
            mensagemP = Codigo.Erro.getMens();
        } else {
            temErro = false;
            mensagemH1 = "Livro atualizado com SUCESSO";
            mensagemP = "O livro '" + chaves.getTitulo() + "' foi salvo no banco de dados.";
        }   
    } catch (Exception e) {
        temErro = true;
    ...
    if (temErro) {
        tituloPagina = "Erro na Atualização";
        cssClass = "error";
    } else {
